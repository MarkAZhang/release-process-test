...
if [[ $NO_DEPLOY == "--no-deploy" ]]; then
...
elif [[ -e "${APP_HOME}/environment.${PROMOTE_DEST_BRANCH}" ]]; then
    source "${APP_HOME}/environment.${PROMOTE_DEST_BRANCH}"
    make -C "$APP_HOME" deploy
    ...
    echo "Error: Could not find environment config file ${APP_HOME}/environment.${PROMOTE_DEST_BRANCH}. Unable to deploy."
    exit 1
fi
